...
$.getJSON('data/hackerIpsum.json')
  .then(
    function(data) {
      ...
      console.log('errors all over', err);
    })
...
Article.fetchAll = function() {
  if (localStorage.rawData) {
    ...
    articleView.initIndexPage(); //TODONE: What do we pass in to loadAll()?
    ...
    $.getJSON('data/hackerIpsum.json')
      .then(
        function(data) {
          ...
          localStorage.setItem('rawData', jsondata); //Putting into local storage
          Article.loadAll(data);
          articleView.initIndexPage();
        }, function (err) {
          console.log('errors all over', err);
